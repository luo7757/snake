...
function Snake(){
	...
	this.directionNum = {//存储蛇的走向，用对象来表示
		left : {
			x : -1,
			y : 0,
			rotate : 180
...
Snake.prototype.getNextPos = function(){
	...
	// 下一个点是自己，代表碰到了自己，游戏结束
	var selfCollied = false;
	this.pos.forEach(function(value){
		if (value[0] == nextPos[0] && value[1] == nextPos[1]) {
			selfCollied = true;
	// 如果数组中的两个数据都相等，就说明下一个点在蛇神上里面能找到，代表撞到了自己
		}
	})
...
Snake.prototype.strategies = {
	move : function(format){//这个参数用于决定要不要删除最后一个方块（蛇尾） 当传了这个参数之后就表示要吃
...
function createFood(){
	...
	var include = true; //循环跳出的条件，true表示食物的坐标在蛇的身上（需要继续循环）。
						//false表示不在蛇的身上（不继续循环）
	while(include){
